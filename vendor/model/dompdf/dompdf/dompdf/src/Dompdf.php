...
class Dompdf
{
    ...
    */
    ...
    public function loadHtml($str, $encoding = 'UTF-8')
    {
        ...
        $metatags = array(
            '@<meta\s+http-equiv="Content-Type"\s+contents="(?:[\w/]+)(?:;\s*?charset=([^\s"]+))?@i',
            '@<meta\s+contents="(?:[\w/]+)(?:;\s*?charset=([^\s"]+))"?\s+http-equiv="Content-Type"@i',
        ...
        if (isset($document_encoding) && in_array(strtoupper($document_encoding), array('UTF-8','UTF8')) === false) {
            ...
            $str = str_replace('<head>', '<head><meta http-equiv="Content-Type" contents="text/html;charset=UTF-8">', $str);
        } elseif (isset($document_encoding) === false) {
            $str = '<meta http-equiv="Content-Type" contents="text/html;charset=UTF-8">' . $str;
        }
    ...
    public function render()
    {
        ...
        if ($options->isFontSubsettingEnabled() && $canvas instanceof CPDF) {
            foreach ($this->tree->get_frames() as $frame) {
                ...
                // Handle generated contents (list items)
                ...
                // Handle other generated contents (pseudo elements)
                // FIXME: This only captures the text of the stylesheet declaration,
                //        not the actual generated contents, and forces all possible counter
                ...
                if ($frame->get_node()->nodeName == "dompdf_generated") {
                    ...
                    // the hex-decoded text of the contents property, duplicated from AbstrctFrameReflower::_parse_string
        ...
        foreach ($metas as $meta) {
            ...
            $value = trim($meta->getAttribute("contents"));
     ...
     *   Apply contents stream compression
     *
     ...
     *
     * 'compress' = > 1 or 0 - apply contents stream compression, this is
     *    on (1) by default
     *
