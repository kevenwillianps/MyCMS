...
try
{
    ...
    $UsersFilesValidate->setPathProfile($resultConfiguration->preferences->file_path_users_files . $UsersFilesValidate->getUserId());
    ...
    else
    {
        ...
        if (@(int)$resultUsers->user_id > 0)
        {
            ...
            if ((int)$FileValidate->getPonteiro() === ((int)$FileValidate->getTamanho() - 1))
            {
                ...
                /** Gero o arquivo */
                $File->generate($UsersFilesValidate->getPathProfile(), $UsersFilesValidate->getName(), $FileValidate->getBase64());

                /** Verifico se o caminho existe */
                if (is_file($UsersFilesValidate->getFullPathProfile()))
                {

                    /** Manipulo as imagens */
                    $File->handling($UsersFilesValidate->getPathProfile(), $UsersFilesValidate->getName(), $resultConfiguration->preferences->images_dimensions);

                    /** Salvo o registro do arquivo */
                    if ($UsersFiles->SaveProfile($UsersFilesValidate->getUserFileId(), $UsersFilesValidate->getUserId(), $UsersFilesValidate->getName(), $UsersFilesValidate->getPathProfile()))
                    {
